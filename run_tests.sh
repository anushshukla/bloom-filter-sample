...
echo "🧪 Bloom Filter System - Comprehensive Test Suite"
echo "================================================="
...
check_prerequisites() {
    ...
    if redis-cli ping >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}⚠️  Redis server is not running (some tests may fail)${NC}"
    fi
    ...
    if [ ! -f "frontend/tests/test_script.js" ]; then
        missing_deps+=("test_script.js")
    fi
...
run_backend_tests() {
    ...
    run_test "Bloom Filter Pure Functions" \
        ...
        "backend/tests/test_bloom_filter.lua"
    
    # Test 2: Redis Adapter
    run_test "Redis Adapter Functions" \
        "lua backend/tests/test_redis_adapter.lua" \
        "backend/tests/test_redis_adapter.lua"
    
    # Test 3: Individual Redis Scripts
    echo -e "${BLUE}🔍 Testing Individual Redis Scripts${NC}"
    echo "====================================="
    
    # Test process_file.lua
    if [ -f "backend/redis_scripts/process_file.lua" ]; then
        echo "Testing process_file.lua syntax..."
        if lua -l backend/redis_scripts/process_file.lua -e "print('Syntax OK')" 2>/dev/null; then
            echo -e "${GREEN}✅ process_file.lua syntax OK${NC}"
            passed_tests=$((passed_tests + 1))
        else
            echo -e "${RED}❌ process_file.lua syntax error${NC}"
            failed_tests=$((failed_tests + 1))
        fi
        total_tests=$((total_tests + 1))
    fi
    
    # Test check_element.lua
    if [ -f "backend/redis_scripts/check_element.lua" ]; then
        echo "Testing check_element.lua syntax..."
        if lua -l backend/redis_scripts/check_element.lua -e "print('Syntax OK')" 2>/dev/null; then
            echo -e "${GREEN}✅ check_element.lua syntax OK${NC}"
            passed_tests=$((passed_tests + 1))
        else
            echo -e "${RED}❌ check_element.lua syntax error${NC}"
            failed_tests=$((failed_tests + 1))
        fi
        total_tests=$((total_tests + 1))
    fi
    
    # Test other Redis scripts
    for script in backend/redis_scripts/*.lua; do
        if [ -f "$script" ]; then
            local script_name=$(basename "$script")
            echo "Testing $script_name syntax..."
            if lua -l "$script" -e "print('Syntax OK')" 2>/dev/null; then
...
run_performance_tests() {
    ...
    # Test 1: Bloom filter creation performance
    if command_exists lua && [ -f "backend/redis_scripts/process_file.lua" ]; then
        echo "Testing bloom filter creation performance..."
        
        # Create test data
        local test_data=""
        for i in {1..1000}; do
            test_data="${test_data}test_item_$i\n"
        done
        
        # Measure performance
        local start_time=$(date +%s.%N)
        echo "$test_data" | lua backend/redis_scripts/process_file.lua "perf_test" 1000 0.01 >/dev/null 2>&1
        local end_time=$(date +%s.%N)
        
        local duration=$(echo "$end_time - $start_time" | bc -l 2>/dev/null || echo "0")
        if (( $(echo "$duration < 5" | bc -l) )); then
            echo -e "${GREEN}✅ Bloom filter creation: ${duration}s (under 5s threshold)${NC}"
            passed_tests=$((passed_tests + 1))
        else
            echo -e "${YELLOW}⚠️  Bloom filter creation: ${duration}s (over 5s threshold)${NC}"
            passed_tests=$((passed_tests + 1))
        fi
        total_tests=$((total_tests + 1))
        
        # Cleanup
        redis-cli --eval backend/redis_scripts/delete_filter.lua "perf_test" >/dev/null 2>&1
    fi
    
    # Test 2: Memory usage
    echo "Testing memory usage..."
    if command_exists lua; then
        local memory_test=$(lua -e "
            local bloom_filter = require 'backend.bloom_filter'
            local filter = bloom_filter.create_bloom_filter_from_file('memory_test', 'test\nline\ndata', 10, 0.01)
            local memory = bloom_filter.estimate_memory_usage(filter.metadata.size)
            print(string.format('%.2f', memory.kilobytes))
        " 2>/dev/null)
        
        if [ -n "$memory_test" ] && (( $(echo "$memory_test < 100" | bc -l) )); then
            echo -e "${GREEN}✅ Memory usage: ${memory_test}KB (under 100KB threshold)${NC}"
            passed_tests=$((passed_tests + 1))
        else
            echo -e "${YELLOW}⚠️  Memory usage: ${memory_test}KB${NC}"
            passed_tests=$((passed_tests + 1))
        fi
        total_tests=$((total_tests + 1))
    fi
